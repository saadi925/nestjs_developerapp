...
type ListCoursesResponse = { data: Course[], total: number, page: number, pages: number }
@Injectable()
export class CourseService {
  ...
  async create(course: CreateCourseDto): Promise<Course> {
   if (course._id) {
    this.updateCourse(course._id, course)
   } else {
  ...
  async listAllCourses(page: number, limit: number, sort : 'desc' | 'asc' , filterString?: string): Promise<ListCoursesResponse> {
  ...
  async findCourseBySlug(slug: string) {
   let course = await this.courseModel.findOne({slug}).select('-videoLectures -reviews -rawData')
   .exec()
   return course
  }
  async findCourseForEdit(slug : string){
    const course = await this.courseModel.findOne({slug}).select('-videoLectures -reviews')
   .exec()
...
const result = zlib.gunzipSync(decodedData).toString();
 return result
  ...
  async findCourseForWatch(slug: string){
    const course = await this.courseModel.findOne({slug}).select('-rawData -content -reviews -thubnail').populate('videoLectures').exec()
  ...
  async watchLecture(courseSlug: string, lectureId: string) {
    ...
    const lecture = course.videoLectures.find(lecture => lecture._id.toString() === lectureId)
    if (!lecture) {
