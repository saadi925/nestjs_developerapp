...
export class MemberShipGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    const requiredMemberships = this.reflector.getAllAndOverride<
      $MemberShipType[]
    >(MEMBERSHIP_KEY, [context.getHandler(), context.getClass()]);
    ...
    return requiredMemberships.some(
      (membership) => user.membership.type === membership,
    );
  }
}
